...
> 登录入口在专家端 :3005 → 管理员 tab（与 web 共享 `token_web` 会话）。
...
- **会话共享例外**：开评标管理端 :3007 读取 `token_web` cookie，与 web :3004 共享同一会话（后端按角色命名 cookie，`admin`/`bid_host` 都落到 `web` 命名空间，无 `token_bid`）。
