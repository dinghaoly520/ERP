...
> **状态**：✅ 已完成（2026-09-22 实施：`handover-preview.tsx` 七段只读预览 + 入口接入；浏览器实测/勾稽矩阵对齐/红牌闸全过。实施纪要：① 验证前提「现成回流包」当日失效——09-22 09:44 业务清空致 `BidSignPacket`/FileAsset 行丢失，MinIO 真包（220KB）在，已按对象实测 sha256 补登记 DB（时序取包内真值）；② 评分矩阵取「供应商×评分项」聚合口径——包内 expertId 无姓名映射源（全包只存姓名），专家维度由 expertConfirmations 表承担，末两行勾稽校验通过；③ 深链 projectId 为 PMI 侧 id `cmtvdx881002xuu2ceb27yesk`（非 BidProject id，误用会静默落回列表页））
