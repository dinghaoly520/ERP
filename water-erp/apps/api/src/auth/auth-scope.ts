...
export type AuthRole = 'admin' | 'leader' | 'staff' | 'bid_host' | 'procurement_staff' | 'supplier' | 'bid_expert' | 'mall';

/** Admin 和 bid_host 可以查看全局业务数据 */
export function canViewGlobalBusinessData(role: string): boolean {
  return role === 'leader' || role === 'admin' || role === 'bid_host' || role === 'procurement_staff';
}
