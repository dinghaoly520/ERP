...
export class AiService {
  ...
  async suggestBusinessTags(input: {
    ...
    const system = [
      '你是采购业务标签严格匹配助手。任务：根据采购项目的具体采购内容、工程类别和专业领域，从给定词表中严格筛选出与该项目直接相关的标签。',
      '规则：',
      '1. 只能从词表中选择，严禁自造或改写标签名称。',
      '2. 选择 3 至 6 个最核心标签——宁可少选，不可滥选。每个标签必须能直接回答"该项目的供应商需要具备什么资质/能力"。',
      '3. 严格适配原则：仅选择与项目标的物(设备/工程/服务)直接匹配的标签；泛化标签(如"物资采购""工程服务")除非无更精确标签可选才考虑。',
      '4. 排除规则：不要选择与项目无直接关联的标签；不要因"可能有关系"而选择；标签的覆盖面应精准而非宽泛。',
      '5. 示例：钻机采购项目→选"钻探设备""地质勘查""岩心钻探""钻探工程"，不选"建筑工程""水利工程""通用设备"。',
      '6. 仅输出严格 JSON：{"tags":["标签A","标签B"]}，不得包含任何其它文字或解释。若真有词表与项目完全不相关，输出{"tags":[]}。',
    ].join('\n');
  ...
  async polishRequirement(
    ...
    const system = `你是一名政府采购「供应商寻源」需求梳理专家。以下需求描述将直接作为供应商库匹配候选供应商的基础参照——系统会据此按「业务标签、经营范围、企业类型、行业资质」筛选候选。因此这份文字必须是一份"供应商画像"，而非招标公告式的项目说明。

核心立场：通篇围绕「我们要找什么样的供应商」来写——供应商应属于什么行业领域、经营什么业务、具备什么类型与资质、有何能力与业绩；项目预算、限价、截止时间等招标要素一句话带过即可，绝不作为主体。行文统一使用对供应商的直接要求句式——以「本次的供应商应当……」「供应商需具备……」「供应商应满足……」为主（例如资质要求一节用"本次的供应商应当具备……"起笔），避免"采购XX设备""预算XX万元"这类项目陈述句式。

各分节的供应商选取视角（非常重要）：
- 【项目概况】：一两句点明采购事项及其所属行业领域，让匹配端知道往哪个业务大类寻源（如"面向地质勘查装备制造领域"）。预算金额等仅作背景带过。
- 【采购范围】：写清需要供应商提供什么，并点明供应商的经营范围应覆盖哪些业务方向（给出能映射到行业标签的关键词，如"地质勘探设备、岩心钻机类产品的制造或销售"）。
- 【资质要求】：供应商画像的核心——供应商类型（制造商/代理商/服务商）、企业类型、行业资质等级、同类业绩门槛、技术能力。尽量使用能映射到业务标签的领域关键词，而非只写具体型号。
- 【特殊要求】：聚焦对供应商的服务能力约束（服务响应、售后、交付、质保）；采购方式/限价/截止时间等流程要素简述即可。

写作要求：
1. 修正语法错误、错别字，专业术语准确
2. 补充行业惯例应有的供应商选取维度（业绩门槛、服务响应、质保、技术能力等），一律表述为"对供应商的要求"
3. 保持原文结构框架（【项目概况】【采购范围】【资质要求】【特殊要求】）与格式
4. 在原文基础上适度充实——不照搬过短的原文，结合行业惯例把各分节展开到合理篇幅，但不为凑篇幅堆砌空话
5. 篇幅适中：项目概况约 80-120 字、采购范围约 100-150 字、资质要求约 100-150 字、特殊要求约 60-100 字，整体 350-500 字，不超过 600 字
6. ★句式硬约束（必须严格遵守）★：每段正文必须以对供应商的直接要求起笔，严禁以陈述项目或流程的句式开头：
   - 【项目概况】用「本次寻源面向……领域/类型的供应商」开头（如"本次寻源面向地质勘查装备制造领域的供应商"）
   - 【采购范围】【资质要求】【特殊要求】统一用「本次的供应商应当……」「供应商需具备……」「供应商应满足……」开头
   - 严禁出现"采购XX设备""预算XX万元""面向XX领域，采购XX""采用竞争性谈判""交付地点XX"等陈述项目或流程的句式作为任何一段的开头
   - 改写示例：原文"采购800型钻机1台套，预算87.3万元" → 改写为"本次的供应商应当具备800型便携式全液压岩心钻机的制造或供应能力，经营范围覆盖地质勘探设备领域"；原文"采用竞争性谈判，最高限价84.5万元" → 改写为"供应商应能在竞争性谈判方式下响应本项目..."
7. ★剔除无意义项目信息★：凡对筛选供应商没有参考价值的纯项目/财务/行政信息一律不要写入——包括但不限于：预算金额、"已纳入年度预算/年度预算内"、交付地点、投资概算、建设规模数字等，这些与"供应商应当具备什么"毫无关系。最高限价、响应截止时间至多在【特殊要求】末尾一句话带过，绝不作为正文要点或独立条目。
...
只输出润色后的文本，不要添加任何解释或标记。`;
  ...
  async polishInitiationField(params: {
    ...
    const isSupplierReq = field === 'supplierRequirements';
    const fieldGuide = field === 'projectReason'
      ? `「立项事由」应说明：项目背景与必要性、采购目标、预期成效，行文正式、逻辑清晰，一般 150-400 字。`
      : `「供方要求」请用一段话简要概括对供应商的核心要求（资质、业绩、技术能力、服务、交付等），内容精炼，一般不超过 150 字，不分条列述、不堆砌细节。`;
    // 供方要求强调简短概括，抑制通用规则中的扩写倾向
    const lengthRule = isSupplierReq
      ? `\n7. 务必用一段连贯的话概括表述，禁止分条、编号或列举；宁缺毋滥，只保留最核心的要求，不得超出 150 字；不得包含限价、预算金额、最高限价、报价等价格限制类描述`
      : '';
...
6. 如果用户原文已经完善，仅做轻微润色即可，不要过度改写${lengthRule}
...
只输出优化后的文本，不要添加任何解释、前后缀或标记。`;
  ...
  async generateNotificationContent(context: {
    ...
    if (ids.length > 0) {
      ...
      // 兜底 origin 由 @water-erp/config 的 PORTS 派生（审计 D：原硬编码 :3004）。
...
${projectsInfo ? '关联项目:\n' + projectsInfo : ''}`,
  ...
  async recommendSuppliers(
    ...
    // 排除已选供应商（重新选取时不重复推荐）
    if (opts.excludedSupplierIds && opts.excludedSupplierIds.length > 0) where.id = { notIn: opts.excludedSupplierIds };
...
2. extractedText 来自 PDF/文档文本抽取，可能因字体编码缺失或 OCR 识别误差而包含少量乱码字符（形如 �、? 或连续的 ???），这些并不是真实内容。请根据上下文语义还原正确的汉字后再纳入分析，例如"提升�孔钻探装备能力"应理解为"提升深孔钻探装备能力"。在 summary 与 fileAnalyses 的输出中一律使用还原后的正确文字，严禁原样保留乱码符号
3. 如果文件内容为空或无法解析，明确说明"文件内容无法解析"
4. 用语专业、简洁，全部使用中文输出
5. 不输出风险提示或下一步建议
6. 项目简报的结束语必须严格按照 currentStage 的 stageKey 判断：
...
`.trim();
  ...
  async generateTenderFieldContent(payload: {
    ...
    const systemPrompt =
      ...
      `涉及天数、百分比、期限、比例、数量、技术参数等可量化指标时，必须根据项目类型、规模、行业惯例直接填入合理具体的数值，禁止使用 ____ 下划线、空格、括号等占位符，也禁止「待定」「另行约定」等模糊表述；唯有最终投标报价金额（供应商需填报的分项报价、总报价）以 ____ 留空供供应商填写。` +
      `只输出正文：不写标题、抬头（如 致：××）或落款签字栏（如 投标人盖章、法定代表人签字、日期）。` +
      `严禁在正文开头重复字段名称作为前缀（例如字段是"项目完成期限"，正文直接写"自合同签订之日起 60 日内完成"，不得写成"项目完成期限：自合同签订之日起 60 日内完成"）。` +
      `现为"${payload.fieldLabel}"字段生成内容。只返回 JSON: {"content": "正文"}，content 字段不得为空字符串。`;
    const userPrompt =
