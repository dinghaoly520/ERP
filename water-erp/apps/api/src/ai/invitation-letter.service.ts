...
/** 居右落款：统一以采购中心名义发出 */
const SIGN_OFF = '四川省水利发展集团有限公司采购中心';
/** 称谓（本邀请书随配置批量下发受邀供应商，不预填具体单位名） */
const SALUTATION = '致：受邀供应商';

const MEDIA_DEFAULT = '四川省水利发展集团有限公司电子招标采购平台（蜀水云采，https://ssyc.scswdc.com）及本项目信息门户';
  ...
  async generate(projectId: string, times: { acquireStart?: string; acquireEnd?: string; bidAt?: string }) {
    ...
    try {
      ...
      const userPrompt = [
        ...
        '- 第一段为引言：以「四川省水利发展集团有限公司采购中心现就〈项目名称〉（项目编码：××）以××方式组织采购……现邀请贵单位参加」起笔；',
        ...
        '项目已知信息：', known,
        '',
        '固定口径：发布媒介与异议渠道若无更具体信息，分别使用："四川省水利发展集团有限公司电子招标采购平台（蜀水云采）及本项目信息门户"与书面异议+平台异议通道+行政监督投诉的规范表述。',
