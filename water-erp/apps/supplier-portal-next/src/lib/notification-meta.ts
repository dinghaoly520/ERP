...
const META: Record<string, NotificationMeta> = {
  ...
  BID_OPENING_SOON: { label: "开标临近", group: "todo", tone: "warning", actionable: true, actionLabel: "进入大厅签到" },
  DECRYPT_WINDOW_CLOSING: { label: "解密窗口临关", group: "todo", tone: "danger", actionable: true, actionLabel: "立即解密" },
  BOND_REFUND_RESULT: { label: "保证金退还", group: "project", tone: "success" },
  CONTRACT_READY_TO_SIGN: { label: "待签署合同", group: "contract", tone: "info", actionable: true, actionLabel: "查看合同" },
  CONTRACT_SIGNED: { label: "合同已签署", group: "contract", tone: "success" },
  PREQUAL_RESULT: { label: "资格预审结果", group: "approval", tone: "accent" },
  BID_OPENING_DECISION: { label: "开标安排", group: "project", tone: "info" },
  HALL_MESSAGE: { label: "会场消息", group: "project", tone: "accent", actionable: true, actionLabel: "进入开标大厅" }, // 点开即阅，无 resolvedAt 闭环，不入 todo 组
  PROJECT_TERMINATED: { label: "项目终止", group: "project", tone: "danger" },
  PASSWORD_RESET_APPROVED: { label: "密码重置结果", group: "approval", tone: "info" },
  SELECTION_NOTIFY: { label: "候选名单通知", group: "project", tone: "accent" }, // 历史类型，存量行展示用
