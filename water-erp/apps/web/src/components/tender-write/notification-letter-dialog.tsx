              ...
              {values.map((v, i) => (
                <td key={i} className="whitespace-nowrap border-b border-white/40 px-3 py-2.5 text-[color:var(--foreground)]">
                  {v ? (
                    ...
                    <span className="text-[color-mix(in_oklch,var(--danger)_50%,transparent)]">—</span>
                  )}
                </td>
              ))}
            </tr>
...
function PreviewField({ value, placeholder }: { value: string; placeholder: string }) {
  if (value.trim()) {
    return <span className="border-b border-[color-mix(in_oklch,var(--success)_25%,transparent)]">{value}</span>;
  }
  return (
    <span className="border-b border-dashed border-[color-mix(in_oklch,var(--danger)_25%,transparent)] text-[color-mix(in_oklch,var(--danger)_55%,transparent)]">
      {placeholder}
    </span>
  );
}
