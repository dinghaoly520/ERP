...
export function AbortDialog({ bidProjectId, projectName, projectCode, isOpen, onClose, onChanged }: Props) {
  ...
  // 执行中（aborting/archiving）禁止 X/背景/Esc 关闭——原版背景点击未设防，执行中误触会半途关掉对话框
  const canDismiss = step === 'idle' || step === 'done' || step === 'error';
  const guardedClose = () => { if (canDismiss) handleClose(); };
        ...
        step === 'done' ? (
          dealFormOpen ? (
            <div className="neu-btn-group">
              <button type="button" onClick={() => setDealFormOpen(false)} className="neu-btn-soft" disabled={dealBusy}>取消</button>
              <button type="button" onClick={handleRegisterDeal} className="neu-btn-primary" disabled={dealBusy || !dealWinner.trim()}>
                {dealBusy ? <Loader2 size={14} className="animate-spin" /> : '登记成交'}
              </button>
            </div>
          ) : (
            <div className="neu-btn-group">
              <button type="button" onClick={handleClose} className="neu-btn-primary">关闭</button>
            </div>
          )
        ) : step === 'error' ? (
          <div className="neu-btn-group">
            <button type="button" onClick={() => setStep('idle')} className="neu-btn-soft">重试</button>
          </div>
        ) : (
          <div className="neu-btn-group">
    ...
    >
      <div className="flex items-center gap-3">
        <div
          ...
          style={{ '--well-bg': 'color-mix(in oklch, var(--danger) 12%, transparent)', '--well-fg': 'var(--danger)' } as React.CSSProperties}
        >
