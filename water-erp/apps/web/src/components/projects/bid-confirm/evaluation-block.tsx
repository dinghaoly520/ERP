...
/**
 * 评标管理区块——:3005 开评标指挥中心 Phase 2 新增。
 * 移植自 bid-portal evaluate/page.tsx：进度仪表盘、启动评标、专家状态卡、
 * 专家×供应商评分矩阵（偏差>20% 标异常）、供应商汇总排名（实时均分参考 /
 * 官方结果）、3 步生成向导。归档由独立区块负责；实时性由父组件 socket 驱动。
 */
...
export function EvaluationBlock({ bidProjectId, detail, onChanged }: Props) {
  ...
  const supplierAvg = useMemo(() => {
    ...
    for (const s of detail.suppliers) {
      const scores = supplierPercentScores(detail, matrix, s.id);
  ...
  const liveRanks = useMemo(() => {
    if (!detail) return new Map<string, number>();
    const entries = [...detail.suppliers].map(s => ({ id: s.id, avg: supplierAvg.get(s.id) ?? 0 }));
  ...
  const anomalies: { expert: BidProjectExpertInfo; supplier: BidProjectSupplierInfo; pct: number; avg: number }[] = [];
  for (const expert of experts) {
  ...
  async function handleStartEvaluation() {
    setBusy(true);
    try {
      await startEvaluation(bidProjectId);
  ...
  async function handleGenerate() {
    setBusy(true);
    try {
      const r = await generateEvaluationResults(bidProjectId);
      setResults(r);
      setWizardOpen(false);
      setWizardStep(0);
      showToast('评标结果已生成');
