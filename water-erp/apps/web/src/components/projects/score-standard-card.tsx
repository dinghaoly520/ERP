 ...
 */
export function ScoreStandardCard({ project, round, bidProject, detail, onChanged }: Props) {
  return (
    <SectionCard
    ...
    >
      {!bidProject ? (
        ...
        <div className="space-y-4">
          {/* ① 评标口径：评标办法 + 最高限价（EVALUATING 起锁定） */}
          <div>
            <h4 className="mb-2 text-[11px] font-bold uppercase tracking-[0.08em] text-[var(--muted-foreground)]">
              评标办法与最高限价
            </h4>
            <EvaluationBasisFields detail={detail ?? bidProject} onChanged={onChanged} />
          </div>
          ...
          {/* ③ 价格分公式参数（作用于「价格」类评分项，留空=内置默认公式） */}
          <hr className="wb-section-rule" />
          <div>
            <h4 className="mb-2 text-[11px] font-bold uppercase tracking-[0.08em] text-[var(--muted-foreground)]">
              价格分公式参数（高级）
            </h4>
            <PriceFormulaFields detail={detail ?? bidProject} onChanged={onChanged} />
          </div>
        </div>
      )}
    </SectionCard>
