...
function Modal({ children, onClose }: { children: ReactNode; onClose: () => void }) {
  return <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}><div className="absolute inset-0 bg-[var(--background)]/60 backdrop-blur-sm"/><div className="relative max-w-[min(480px,92vw)] w-full max-h-[80vh] overflow-y-auto rounded-[20px] bg-[var(--background)] p-6 shadow-[0_20px_60px_rgba(0,0,0,0.12)]" onClick={e=>e.stopPropagation()}>{children}</div></div>;
}
...
function TrendChartPanel({ profile, index, reducedMotion }: { profile: DashboardData; index: number; reducedMotion: boolean }) {
  ...
  return (
    <>
      ...
      {activeTrend && <Modal onClose={()=>setActiveTrend(null)}><div className="flex items-start justify-between pb-4"><div className="flex items-center gap-3"><CalendarRange size={20} className="text-[var(--accent)]"/><div><div className="text-base font-semibold tracking-[-0.03em] text-[var(--foreground)]">{activeTrend.label}</div><div className="mt-0.5 flex items-center gap-2"><span className="text-[11px] font-bold text-[var(--accent)]">{activeTrend.count} 项</span><span className="text-xs text-[var(--muted-foreground)]">成交金额 {activeTrend.amount.toFixed(1)}万</span></div></div></div><button onClick={()=>setActiveTrend(null)} className="neu-btn-xs"><X size={16}/></button></div><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.12em] text-[var(--muted-foreground)] mb-2"><FolderKanban size={12}/> 当日项目</div><div className="space-y-1.5">{(activeTrend.projects??[]).map((p,idx)=><div key={idx} className="neu-card-static rounded-[10px] flex items-center gap-3 px-3 py-2"><div className="flex-1 min-w-0"><div className="truncate text-[11px] font-medium text-[var(--foreground)]">{p.name}</div><div className="mt-0.5 flex items-center gap-2 text-xs text-[var(--muted-foreground)]"><span>{p.department}</span><span>·</span><span>{p.method}</span></div></div><div className="text-right shrink-0"><div className="text-xs font-bold text-[var(--foreground)]">{p.awardLabel||p.budgetLabel}</div><div className={`text-xs ${sc(p.status)}`}>{p.status}</div></div></div>)}</div></Modal>}
    </>
  );
}
