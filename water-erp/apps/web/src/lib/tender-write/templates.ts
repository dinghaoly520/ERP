    ...
    fields: [
      {
        ...
        multiline: true,
        aiPrompt: "根据项目名称生成项目概况和采购内容，包含采购背景、目标、范围、具体采购内容等关键信息。要求：根据内容复杂度选择合适的表达方式。简单内容直接用段落描述，无需编号；复杂或多项内容可用层次编号（主层次用1.、2.、3.，子层次用①②③）。不要过度使用编号，避免机械感。不要使用#、*等符号，不要出现空行，不要在开头重复字段名称。",
      },
      {
        key: "maxPrice",
        label: "最高限价",
      ...
      },
      {
        ...
        multiline: true,
        aiPrompt: "根据项目类型生成特定资格要求，包含资质证书、业绩要求、人员配置等。要求：根据内容复杂度选择合适的表达方式。简单内容直接用段落描述，无需编号；复杂或多项内容可用层次编号（主层次用1.、2.、3.，子层次用①②③）。不要过度使用编号，避免机械感。不要使用#、*等符号，不要出现空行，不要在开头重复字段名称。",
      },
      {
      ...
      },
      {
        ...
        label: "响应文件提交截止时间",
        placeholder: "另行通知",
        aiPrompt: "生成响应文件提交截止时间。规则：从今天起往后推3-5个工作日（跳过周六日），格式为'YYYY年MM月DD日'，只输出日期，不要其他说明。",
        composite: {
      ...
      { key: "contactPhone", label: "联系电话", placeholder: "请输入联系电话", type: "tel" },
      { key: "contactEmail", label: "联系邮箱", placeholder: "请输入联系邮箱", type: "email" },
    ],
  },
  {
    key: "supplier",
    title: "供应商须知",
    description: "填写合同分包、现场踏勘和合同文本。",
    fields: [
      {
    ...
    fields: [
      {
        ...
        placeholder: "请输入商务要求",
        multiline: true,
        aiPrompt: "根据项目类型生成商务要求，包含交付期限、付款方式、验收标准、售后服务等。要求：根据内容复杂度选择合适的表达方式。简单内容直接用段落描述，无需编号；复杂或多项内容可用层次编号（主层次用1.、2.、3.，子层次用①②③）。不要过度使用编号，避免机械感。不要使用#、*等符号，不要出现空行，不要在开头重复字段名称。",
      },
      {
        ...
        placeholder: "请输入技术要求",
        multiline: true,
        aiPrompt: "根据项目类型生成技术要求，包含技术标准、性能参数、质量要求等。要求：根据内容复杂度选择合适的表达方式。简单内容直接用段落描述，无需编号；复杂或多项内容可用层次编号（主层次用1.、2.、3.，子层次用①②③）。不要过度使用编号，避免机械感。不要使用#、*等符号，不要出现空行，不要在开头重复字段名称。",
      },
    ],
  },
  {
    ...
    fields: [
      {
        ...
        aiPrompt: "以招标文件的角度撰写报价表要求，明确供应商需要提供的报价内容格式、报价明细要求、报价包含的费用项目、报价币种和单位等。要求：根据内容复杂度选择合适的表达方式，条理清晰便于供应商理解和响应。简单内容直接用段落描述，无需编号；复杂或多项内容可用层次编号。不要过度使用编号，避免机械感。不要使用#、*等符号，不要出现空行，不要在开头重复字段名称。",
      },
    ],
  },
];
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "documentPrice", label: "采购文件售价", placeholder: "例如 0 元" },
      {
        ...
        label: "递交和谈判时间",
        placeholder: "另行通知",
        aiPrompt: "生成递交和谈判时间。规则：从今天起往后推3-5个工作日（跳过周六日），格式为'YYYY年MM月DD日HH时MM分'（如'2026年05月28日09时00分'），只输出一个具体时间，不要其他说明。",
        composite: {
    ...
    fields: [
      {
        ...
        placeholder: "请输入采购内容",
        multiline: true,
        aiPrompt: "根据项目信息生成采购内容，详细描述采购的具体物品、服务或工程内容。要求：根据内容复杂度选择合适的表达方式。简单内容直接用段落描述，无需编号；复杂或多项内容可用层次编号（主层次用1.、2.、3.，子层次用①②③）。不要过度使用编号，避免机械感。不要使用#、*等符号，不要出现空行，不要在开头重复字段名称。",
      },
      {
        ...
        placeholder: "请输入采购要求",
        multiline: true,
        aiPrompt: "根据采购内容生成采购要求，包含质量标准、交付要求、验收条件等。要求：根据内容复杂度选择合适的表达方式。简单内容直接用段落描述，无需编号；复杂或多项内容可用层次编号（主层次用1.、2.、3.，子层次用①②③）。不要过度使用编号，避免机械感。不要使用#、*等符号，不要出现空行，不要在开头重复字段名称。",
      },
    ],
  },
  {
    ...
    fields: [
      {
        ...
        select: {
          options: [
            { value: "（根据采购谈判结果另行拟定）", label: "根据采购谈判结果另行拟定" },
            { value: "（见附件）", label: "见附件" },
          ],
        },
      },
    ...
    fields: [
      {
        ...
        aiPrompt: "以招标文件的角度撰写报价函要求，明确供应商需要提供的报价内容格式、报价明细要求、报价包含的费用项目、报价币种和单位等。使用规范的专业术语，条理清晰，便于供应商理解和响应。",
      },
    ],
  },
];
    ...
    fields: [
      {
        ...
        placeholder: "请输入项目介绍",
        multiline: true,
        aiPrompt: "根据项目名称生成项目介绍，包含项目背景、目标、范围等信息。要求：根据内容复杂度选择合适的表达方式。简单内容直接用段落描述，无需编号；复杂或多项内容可用层次编号（主层次用1.、2.、3.，子层次用①②③）。不要过度使用编号，避免机械感。不要使用#、*等符号，不要出现空行，不要在开头重复字段名称。",
      },
      {
        key: "procurementContent",
        label: "采购内容",
      ...
      },
      {
        ...
        multiline: true,
        aiPrompt: "根据项目类型生成供应商需要提供的资料清单。要求：根据内容复杂度选择合适的表达方式。简单内容直接用段落描述，无需编号；复杂或多项内容可用层次编号（主层次用1.、2.、3.，子层次用①②③）。不要过度使用编号，避免机械感。不要使用#、*等符号，不要出现空行，不要在开头重复字段名称。",
      },
      {
