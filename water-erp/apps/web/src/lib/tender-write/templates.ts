    ...
    fields: [
      {
        ...
        placeholder: "请输入商务要求",
        multiline: true,
        aiPrompt: "根据项目类型生成商务要求，包含交付期限、付款方式、验收标准、售后服务等。要求：根据内容复杂度选择合适的表达方式。简单内容直接用段落描述，无需编号；复杂或多项内容可用层次编号（主层次用1.、2.、3.，子层次用①②③）。不要过度使用编号，避免机械感。不要使用#、*等符号，不要出现空行，不要在开头重复字段名称。若采购事项较多、适合以表格呈现，则以制表符（Tab 键）分隔各项各列，列序为 名称、规格型号、单位、数量、单价、合价（单价和合价留空），不写表头行；若事项较少或更适合段落描述，直接在正文中逐项说明即可。由你判断哪种方式更合适。【关键要求】所有涉及天数、百分比、期限、比例、金额、数量等可量化指标的条款，必须根据项目类型、规模、行业惯例直接填入合理具体的数值，禁止出现下划线占位符（如____、空格留白、横线、括号待填等），也禁止出现「待定」「另行约定」等模糊表述。例如：交付期限根据项目规模填「合同签订后30日内」或「60日历天」；违约金按行业惯例填「每日按合同总价的1‰」；付款方式填「预付款30%、到货验收后付至90%、质保金10%」；质保期根据设备类型填「12个月」或「24个月」；响应时间填「2小时内响应、24小时内到场」。数值须具体且符合该类项目的实际惯例，不要全部使用相同默认值，应根据项目特征差异化设置。",
      },
      {
        ...
        placeholder: "请输入技术要求",
        multiline: true,
        aiPrompt: "根据项目类型生成技术要求，包含技术标准、性能参数、质量要求等。要求：根据内容复杂度选择合适的表达方式。简单内容直接用段落描述，无需编号；复杂或多项内容可用层次编号（主层次用1.、2.、3.，子层次用①②③）。不要过度使用编号，避免机械感。不要使用#、*等符号，不要出现空行，不要在开头重复字段名称。若采购事项较多、适合以表格呈现，则以制表符（Tab 键）分隔各项各列，列序为 名称、规格型号、单位、数量、单价、合价（单价和合价留空），不写表头行；若事项较少或更适合段落描述，直接在正文中逐项说明即可。由你判断哪种方式更合适。【关键要求】所有涉及技术参数、规格、数值、公差、等级等可量化指标的条款，必须根据项目类型、设备/服务特征、行业规范直接填入合理具体的数值，禁止出现下划线占位符（如____、空格留白、横线、括号待填等），也禁止出现「待定」「详见厂家资料」「按相关标准」等模糊或推诿表述。例如：设备参数填「额定功率75kW」「钻进深度≥800m」「精度等级±0.5%」；材料规格填「DN200」「壁厚6mm」「抗压强度C30」；性能指标填「处理水量≥100m³/h」「效率≥95%」「防护等级IP65」。数值须具体且符合该类项目的技术规范，可根据项目名称中体现的规模和型号进行合理推断，不要全部使用相同默认值。",
      },
    ],
  },
  {
