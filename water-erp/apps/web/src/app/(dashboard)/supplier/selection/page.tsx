...
import { Sparkles, Wand2, Copy, Download, X, Plus, FileSearch, ChevronDown, ChevronUp, Filter, Target, Award, Zap, Building2, Briefcase } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          {/* Project association */}
          <SectionCard title="项目关联（可选，用于丰富 AI 上下文）">
          ...
          {/* Requirement */}
          <SectionCard title="采购需求描述 *" description="描述越详细，AI 匹配越精准">
            <textarea value={requirement} onChange={e => setRequirement(e.target.value)}
              placeholder={PROMPT_TEMPLATE}
              className="w-full rounded-xl border border-[#dce6f3] px-3 py-2.5 text-sm h-40 resize-y focus:outline-none focus:border-[#064ea2] font-mono text-xs leading-relaxed placeholder-[#94a3b8] mb-3" />
          ...
          {/* Advanced filters */}
          <SectionCard
            title="高级筛选" description="分类 · 企业类型 · 评分门槛"
            action={
              <button onClick={() => setShowAdvanced(!showAdvanced)}
                className="rounded-lg p-1.5 text-[#8a99ad] hover:bg-[#f8fafc] transition">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input type="range" min={0} max={100} step={5} value={minScore}
                    onChange={e => setMinScore(Number(e.target.value))} className="w-full accent-[#064ea2]" />
          ...
          {result && !loading && (
            <div className="space-y-4">
              ...
              {result.recommendations.map((r, idx) => {
                ...
                const contact = r.contacts?.find(c => c.isPrimary) || r.contacts?.[0];
                return (
                  <div key={r.supplierId} className={`rounded-2xl border bg-white p-5 transition ${
