...
export default function ExpertRepositoryPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await listExperts({ search: query || undefined, specialty: specialty || undefined, page, pageSize: PAGE_SIZE, companyId });
  ...
  useEffect(() => {
    ...
    fetchExpertCompanyCounts({ search: query || undefined, specialty: specialty || undefined })
      .then(setCompanyCounts)
      .catch(() => setCompanyCounts(null));
  }, [isAdmin, companyId, query, specialty]);
  ...
  // admin 全部公司视图：当前页行按公司分组（CompanySectionHeader 台账/进度同款）；
  ...
  const expertGroups = useMemo(() => {
    ...
    return buildCompanyCounts(displayedExperts.map(e => ({ company: e.company }))).map(g => ({
      name: g.name,
      fullCount: companyCounts?.find(c => c.name === g.name)?.count ?? g.count,
      items: displayedExperts.filter(e => ((e.company ?? '').trim() || '未归属') === g.name),
    }));
  }, [companyViewAll, displayedExperts, companyCounts]);

  // 表格渲染函数：分组视图按组各渲染一张表，单表视图整页一张
  const renderExpertTable = (rows: ExpertListItem[]) => (
      ...
      {companyViewAll && !loading && !errored && displayedExperts.length > 0 ? (
        ...
        <>
          ...
          {isAdmin && companyId !== 'all' && selectedCompanyName && (
            <div className="mb-3">
              <CompanySectionHeader name={selectedCompanyName} count={total} />
            </div>
          )}
          {renderExpertTable(displayedExperts)}
        </>
      )}

      {/* 分页（表格外独立一次，分组视图不重复） */}
      {!loading && !errored && displayedExperts.length > 0 && (
