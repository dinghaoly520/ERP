...
export default function ArchiveError({ error, reset }: { error: Error; reset: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-24 text-center">
      ...
      <h2 className="text-sm font-bold text-[#18243a] mb-2">归档端加载失败</h2>
      <p className="text-xs text-[#8a96aa] mb-6 max-w-md">
        {error.message || '发生未知错误，请重试'}
      </p>
      <button
        onClick={reset}
        className="inline-flex items-center gap-1.5 rounded-xl bg-[#11a874] px-4 py-2 text-xs font-bold text-white hover:bg-[#0f9f6e] transition"
      >
