      ...
      <div className="grid grid-cols-[1fr_380px] gap-6">
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-[oklch(0.91_0.006_264)] overflow-hidden">
            <div className="bg-gradient-to-r from-[#043f88] to-[#0e62d0] p-6 text-white">
              ...
              {editing ? (
                <div className="space-y-4">
                  <div>
                    ...
                    <input value={form.displayName} onChange={e => setForm(f => ({ ...f, displayName: e.target.value }))}
                      className="w-full border border-[oklch(0.91_0.006_264)] rounded-lg px-4 py-2.5 text-sm focus:border-[#064ea2] focus:ring-1 focus:ring-[#064ea2] outline-none" />
                  </div>
                  <div>
                  ...
                  <div className="flex gap-3 pt-2">
                    <button onClick={handleSave} disabled={saving}
                      className="px-5 py-2 bg-[#064ea2] text-white rounded-lg text-sm font-semibold hover:bg-[#6d28d9] transition disabled:opacity-50">
                      {saving ? '保存中...' : '保存'}
                    </button>
                ...
                <div className="space-y-4">
                  ...
                  <button onClick={() => setEditing(true)}
                    className="px-4 py-2 bg-purple-50 text-[#064ea2] text-sm font-semibold rounded-lg border border-purple-100 hover:bg-purple-100 transition">
                    ✏️ 编辑资料
                  </button>
                </div>
              )}
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              { label: '参与项目', value: totalProjects, color: '#064ea2' },
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-[oklch(0.91_0.006_264)] p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => router.push('/projects')} className="text-xs text-[#064ea2] hover:underline font-semibold">查看全部</button>
            </div>
                ...
                {profile.assignments.map(a => (
                  <div key={a.id}
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded flex-shrink-0 ${a.progress >= 100 ? 'bg-emerald-50 text-emerald-600' : 'bg-purple-50 text-[#064ea2]'}`}>
                    ...
                    <div className="h-1.5 bg-[oklch(0.94_0.004_264)] rounded-full overflow-hidden">
                      <div className={`h-full rounded-full transition-all duration-500 ${a.progress >= 100 ? 'bg-gradient-to-r from-emerald-500 to-emerald-400' : 'bg-gradient-to-r from-[#064ea2] to-[#0e62d0]'}`}
