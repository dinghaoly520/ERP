 ...
 *
 * 公开路由（proxy.ts 放行）——进入时平板尚无会话，票据即入口凭证。
 */
...
function ClaimFlow() {
      ...
      <div className="mx-auto max-w-md space-y-4 px-6 pt-8 pb-12">
        <div className="exp-alert exp-alert--success flex items-center gap-2 !p-4">
          ...
          <div>
            ...
            <p className="text-xs opacity-80">请拍摄留档照后进入评审</p>
          </div>
        </div>
        <div className="neu-card-static p-4">
          ...
          >
            暂不拍摄，直接进入评审 <ArrowRight size={13} strokeWidth={1.7} />
          </button>
        </div>
      </div>
    );
  ...
  const submit = async () => {
    ...
    setError(null);
    try {
      const r = await api.post<{ username: string; projectId: string }>('/auth/expert-transfer/claim', {
        ticket,
        password,
      });
