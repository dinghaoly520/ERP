...
function SplashCursor({
  ...
  const paletteKey = Array.isArray(COLOR_PALETTE) ? COLOR_PALETTE.join('|') : '';

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const resolvedColorPalette = paletteKey ? paletteKey.split('|') : undefined;
    ...
    function generateColor() {
      ...
      if (Array.isArray(config.COLOR_PALETTE) && config.COLOR_PALETTE.length > 0) {
        const paletteIndex = Math.floor(Math.random() * config.COLOR_PALETTE.length);
        return hexToRGB(config.COLOR_PALETTE[paletteIndex]);
      }
