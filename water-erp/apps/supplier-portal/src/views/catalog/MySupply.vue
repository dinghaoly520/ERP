...
async function load() {
  loading.value = true
  try {
    supply.value = await catalogApi.listSupply() as any
  } finally {
    loading.value = false
  }
}
    ...
    <div v-else class="supply-grid">
      <div v-for="s in supply" :key="s.id" class="supply-card">
        <div class="supply-card-head">
          ...
          <el-tag :type="s.status === 'ACTIVE' ? 'success' : 'info'" effect="light" size="small">
...
.supply-card-body { padding: 14px 16px; }
...
.supply-price-value { font-size: 22px; color: #dc2626; font-weight: 800; }
