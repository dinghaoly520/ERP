...
async function load() {
  loading.value = true
  try { supply.value = await catalogApi.listSupply() as any }
  finally { loading.value = false }
}

function openUpdate(s: any) {
  dialogItem.value = { id: s.catalogItemId, name: s.catalogItem.name, code: s.catalogItem.code, specification: s.catalogItem.specification, unit: s.catalogItem.unit }
...
<template>
  <div class="page-container" v-loading="loading">
    ...
    <div v-if="supply.length === 0 && !loading" class="sp-empty" style="padding: 64px;">
    ...
    <div v-else class="supply-grid">
      <div v-for="s in supply" :key="s.id" class="supply-card">
        <div class="supply-card-head">
          <div>
            <div class="supply-code">{{ s.catalogItem.code }}</div>
            <div class="supply-name">{{ s.catalogItem.name }}</div>
            <div class="supply-spec">{{ s.catalogItem.specification }}</div>
          </div>
          <span class="sp-status" :class="s.status === 'ACTIVE' ? 'approved' : 'disabled'">
            {{ s.status === 'ACTIVE' ? '供货中' : '已停用' }}
          </span>
        </div>
        <div class="supply-card-body">
          <div class="supply-price">
            <span class="supply-price-label">当前报价</span>
            <span class="supply-price-value">&yen;{{ Number(s.quotedPrice).toLocaleString() }}<small> / {{ s.catalogItem.unit }}</small></span>
          </div>
          <div class="supply-meta">
            <span v-if="s.deliveryPeriod">交期 {{ s.deliveryPeriod }}</span>
            <span v-if="s.region"> &middot; {{ s.region }}</span>
            <span v-if="s.minOrder"> &middot; 起订 {{ s.minOrder }}</span>
          </div>
          <div class="supply-time">更新于 {{ dayjs(s.updatedAt).format('YYYY-MM-DD') }}</div>
        </div>
        <div class="supply-card-foot">
          <el-button v-if="s.status === 'ACTIVE'" size="small" type="primary" plain @click="openUpdate(s)">申请改报价</el-button>
        </div>
      </div>
    </div>
...
.supply-card-head {
  ...
  padding: 16px 18px; border-bottom: 1px solid var(--sp-border-light);
}
...
.supply-card-foot {
  padding: 12px 18px; border-top: 1px solid var(--sp-border-light);
