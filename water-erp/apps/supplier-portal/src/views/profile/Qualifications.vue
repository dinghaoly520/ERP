...
onMounted(async () => {
  try { await supplierStore.fetchQualifications() }
...
async function customUpload(options: any) {
  ...
  if (file.size > 50 * 1024 * 1024) { ElMessage.error('文件不能超过 50MB'); options.onError(new Error('FILE_TOO_LARGE')); return }
  uploading.value = true
  try {
    const res = await uploadFile(file, 'qualification')
    form.value.fileUrl = res.url; uploadedMeta.value = res
    options.onSuccess(res); ElMessage.success('文件上传成功')
...
async function handleAdd() {
  ...
  try { await supplierStore.addQualification(form.value); ElMessage.success('资质材料添加成功'); dialogVisible.value = false }
...
async function handleDelete(id: string) {
  ...
  try { await supplierStore.deleteQualification(id); ElMessage.success('已删除') }
...
<template>
  <div class="page-container" v-loading="loading">
    <div class="sp-page-title-row">
      <div>
        <div class="sp-page-eyebrow">Qualifications</div>
      ...
      <el-col :xs="24" :sm="12" :lg="8" v-for="q in supplierStore.qualifications" :key="q.id">
        <div class="sp-card qual-card">
          <div class="qual-card-top">
            ...
            <span class="sp-status" :class="getStatusInfo(q).cls">{{ getStatusInfo(q).label }}</span>
          </div>
          <h3 class="qual-name">{{ q.name }}</h3>
          <div class="qual-meta">
            <div v-if="q.validFrom" class="qual-date">
    ...
    <div v-else class="sp-card">
      <div class="sp-empty">
        <div class="sp-empty-icon"><el-icon :size="22"><Folder /></el-icon></div>
        <div class="sp-empty-text">暂无资质材料</div>
        <div class="sp-empty-desc">点击上方按钮添加您的企业资质证照</div>
      </div>
    </div>
    ...
    <el-dialog v-model="dialogVisible" title="添加资质材料" width="520px" destroy-on-close>
      <el-form :model="form" label-width="100px" size="large">
        ...
        <el-form-item label="上传文件" required>
          <el-upload :show-file-list="false" :http-request="customUpload" accept=".pdf,.jpg,.jpeg,.png,.gif,.webp,.doc,.docx,.xls,.xlsx,.zip,.txt">
          ...
          <span v-else class="upload-hint">支持 PDF、图片、Office、ZIP，&le;50MB</span>
        </el-form-item>
      </el-form>
      <template #footer>
