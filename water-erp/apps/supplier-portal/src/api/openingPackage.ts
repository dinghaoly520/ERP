...
/* ═══ 双信封 v2 供应商解密包（T17，§5.3）═══
   - GET opening-package：C_inner 下载凭证 + kselfByRole + sealedFields + 窗口状态。
     轮询场景传 { silent: true } 避免 400 每 10s 弹全局 toast（业务码由开标大厅卡片消化）。
...
/** 显式声明 Promise<any>：axios 拦截器已解包 response.data（TS 静态类型仍为 AxiosResponse，见 api/index.ts） */
export function getOpeningPackage(projectId: string): Promise<any> {
  return api.get(`/supplier-portal/bid-submissions/${projectId}/opening-package`, { silent: true })
}
...
export function decryptUpload(projectId: string, form: FormData): Promise<any> {
  return api.post(`/supplier-portal/bid-submissions/${projectId}/decrypt-upload`, form, {
    headers: { 'Content-Type': 'multipart/form-data' },
    timeout: 120000, // 明文 50MB×4 全量上传，沿用 upload.ts 120s 口径
  })
}
