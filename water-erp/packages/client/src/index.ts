...
export interface ApiClientOptions {
  /** 门户标识，注入 X-Portal 头（后端 cookie 命名空间解析依据） */
  portal: string;
  /** API 基址；默认 '/api'（各 Next 门户经 rewrites/middleware 代理到 :4001） */
  baseUrl?: string;
  /** 401 回调（通常是跳转登录页）。仅浏览器端触发；SSR 下静默由调用方 catch ApiError */
  on401?: () => void;
}
  ...
  return {
    ...
    post: <T>(path: string, body?: unknown, init?: RequestInit) =>
      fetchApi<T>(path, {
        ...init,
        method: 'POST',
        headers: mergeHeaders(init, { 'Content-Type': 'application/json' }),
        body: body === undefined ? undefined : JSON.stringify(body),
      }),
    postForm: <T>(path: string, body: FormData) =>
      fetchApi<T>(path, { method: 'POST', body }),
    put: <T>(path: string, body?: unknown) =>
      fetchApi<T>(path, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: body === undefined ? undefined : JSON.stringify(body) }),
    patch: <T>(path: string, body?: unknown) =>
      fetchApi<T>(path, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: body === undefined ? undefined : JSON.stringify(body) }),
