...
// 数字计数动画
const countEls = [...document.querySelectorAll('.procurement-stats [data-count]')];
let counted = false;
...
// 进入中心/登录/注册模态框
const procurementEnterTemplate = () => `
  <h3>进入采购管理中心</h3>
  <p>请选择您的身份进入系统</p>
  <div style="display: grid; gap: 12px; margin-top: 20px;">
    <button class="btn btn-solid" style="width: 100%; background: #7c3aed;" onclick="alert('采购人入口 - 演示')">采购人入口</button>
    <button class="btn btn-solid" style="width: 100%; background: #7c3aed;" onclick="alert('审批人入口 - 演示')">审批人入口</button>
    <button class="btn btn-solid" style="width: 100%; background: #7c3aed;" onclick="alert('管理员入口 - 演示')">管理员入口</button>
  </div>
...
const accountTemplate = type => `
  ...
  <p>${type === 'login' ? '登录采购管理中心，开始智慧采购。' : '注册成为平台用户，体验智慧采购管理。'}</p>
  <form class="modal-form" id="demoForm">
    ...
    <div class="modal-actions">
      <button class="btn btn-solid" type="submit" style="background: #7c3aed;">${type === 'login' ? '登录' : '提交注册'}</button>
...
const consultTemplate = () => `
  <h3>联系咨询</h3>
  <p>如需了解更多采购管理信息，欢迎联系我们。</p>
  <div class="consult-info" style="margin-top: 20px;">
    <div style="display: grid; gap: 16px;">
      <div style="display: flex; align-items: center; gap: 12px;">
        <span style="font-size: 20px;">📞</span>
        <div><strong>咨询热线</strong><br/><span style="color: #7c3aed; font-weight: 700;">400-888-8888</span></div>
      </div>
      <div style="display: flex; align-items: center; gap: 12px;">
        <span style="font-size: 20px;">📧</span>
        <div><strong>邮箱</strong><br/><span>procurement@scwater.com</span></div>
      </div>
      <div style="display: flex; align-items: center; gap: 12px;">
        <span style="font-size: 20px;">⏰</span>
  ...
  <div class="modal-actions">
    <button class="btn btn-solid" data-close style="background: #7c3aed;">我知道了</button>
  </div>
`;
...
document.querySelectorAll('[data-open]').forEach(btn => {
  btn.addEventListener('click', e => {
    ...
    if (type === 'procurement-enter') {
      openModal(procurementEnterTemplate());
...
document.querySelectorAll('.stat-item, .module-card, .flow-step, .advantage-card, .type-card').forEach(el => {
