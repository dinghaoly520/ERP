...
- **D（2026-07-03）✅ 后端已落地**：抽 `utils/archive-ai-usage.ts:buildArchiveAiUsage` 纯函数 + schema 加 `aiProvenance` + `task.service` create 写快照 + `exportArchivePackage` 加「AI 辅助说明」节(JSON+CSV)；集中 `PROMPT_VERSIONS` 而非每文件。5 单测通过。migration ✅ 已 apply。
- **P1-E（2026-07-06）✅ 后端已落地**：schema `BidScoreDelta` + `submitScores` 事务写 delta（数值项，排除通过性项）+ `confirmReport` 置 `expertReportConfirmed` + `getAiAdoption` API（项目级采纳率 + 按评分项）。migration ✅ 已 apply。**待做**：calibration / expert-deviation API + web 驾驶舱 MetricCard。⏸️ **E-web 暂缓**：`DashboardHome`（2837 行）无项目上下文/api，ai-adoption 项目级不匹配全局驾驶舱；需先补 calibration API（全局）+ DashboardHome 重构，单独一轮。
- **P1-F（2026-07-06）✅ 后端已落地**：`clarification-ai.service`（`draftQuestion` 起草候选问题 + `summarizeReply` 提炼要点）+ 两端 draft 端点（expert + bid，不落库）+ bid summarize 端点（写 `BidClarification.aiSummary`）；`@Optional` 注入（P1-F 是增强，不阻塞 BidService DI，避免 spec 10 个 TestingModule 加 mock）。schema 加 `aiSummary` + migration ✅。bid+expert 300 tests 通过。✅ **前端已接（2026-07-07）**：bid-portal 发起 form「AI 起草」按钮 + 回复列 aiSummary 块 + 「AI 摘要」按钮；expert-portal 澄清 panel 发起 form「AI 起草」按钮。两端 tsc 通过。
- **expert-admin pre-existing（2026-07-06）✅ 修复**：`portrait & retire` 4 失败测试——mock 缺 `user.update`/`expertProfile.updateMany`/findMany 默认值；测试 mock `findFirst` 但逻辑用 `findMany`；`expertEvaluation` mock 缺 `expertUserId`（group key）。12/12 全过。
